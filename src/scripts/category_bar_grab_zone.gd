...
var category_bar = preload("res://src/scenes/category_bar_piece.tscn")
var root_scene
var category_bar_icon_node
var category_bar_instance
...
func _ready():
	...
	category_bar_instance = category_bar.instantiate()
	category_bar_icon_node = get_node("Small Category Bar")
...
func _on_area_2d_input_event(viewport, event, shape_idx):
	if Input.is_action_just_pressed("click"):
		if made_instance == false:
			made_instance = true
			root_scene.add_child(category_bar_instance)
			category_bar_icon_node.hide()
			category_bar_instance.position = get_global_mouse_position()
	if Input.is_action_just_released("click"):
		if made_instance == true and area_entered == true:
			made_instance = false
			root_scene.remove_child(category_bar_instance)
			category_bar_icon_node.show()
			category_bar_instance.delete_instance()
