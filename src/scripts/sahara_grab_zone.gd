...
var sahara = preload("res://src/scenes/sahara_piece.tscn")
var root_scene
var sahara_icon_node
var sahara_instance
...
func _ready():
	...
	sahara_instance = sahara.instantiate()
	sahara_icon_node = get_node("Small Sahara")
...
func _on_area_2d_input_event(viewport, event, shape_idx):
	if Input.is_action_just_pressed("click"):
		if made_instance == false:
			made_instance = true
			root_scene.add_child(sahara_instance)
			sahara_icon_node.hide()
			sahara_instance.position = get_global_mouse_position()
	if Input.is_action_just_released("click"):
		if made_instance == true and area_entered == true:
			made_instance = false
			root_scene.remove_child(sahara_instance)
			sahara_icon_node.show()
			sahara_instance.delete_instance()
