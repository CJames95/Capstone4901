...
var location = preload("res://src/scenes/location_piece.tscn")
var root_scene
var location_icon_node
var location_instance
...
func _ready():
	...
	location_instance = location.instantiate()
	location_icon_node = get_node("Small Location")
...
func _on_area_2d_input_event(viewport, event, shape_idx):
	if Input.is_action_just_pressed("click"):
		if made_instance == false:
			made_instance = true
			root_scene.add_child(location_instance)
			location_icon_node.hide()
			location_instance.position = get_global_mouse_position()
	if Input.is_action_just_released("click"):
		if made_instance == true and area_entered == true:
			made_instance = false
			root_scene.remove_child(location_instance)
			location_icon_node.show()
			location_instance.delete_instance()
