...
var shopping_bag = preload("res://src/scenes/shopping_bag_piece.tscn")
var root_scene
var shopping_bag_icon_node
var shopping_bag_instance
...
func _ready():
	...
	shopping_bag_instance = shopping_bag.instantiate()
	shopping_bag_icon_node = get_node("Small Shopping Bag")
...
func _on_area_2d_input_event(viewport, event, shape_idx):
	if Input.is_action_just_pressed("click"):
		if made_instance == false:
			made_instance = true
			root_scene.add_child(shopping_bag_instance)
			shopping_bag_icon_node.hide()
			shopping_bag_instance.position = get_global_mouse_position()
	if Input.is_action_just_released("click"):
		if made_instance == true and area_entered == true:
			made_instance = false
			root_scene.remove_child(shopping_bag_instance)
			shopping_bag_icon_node.show()
			shopping_bag_instance.delete_instance()
