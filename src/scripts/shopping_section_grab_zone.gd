...
var shopping_section = preload("res://src/scenes/shopping_section_piece.tscn")
var root_scene
var shopping_section_icon_node
var shopping_section_instance
...
func _ready():
	...
	shopping_section_instance = shopping_section.instantiate()
	shopping_section_icon_node = get_node("Small Shopping Section")
...
func _on_area_2d_input_event(viewport, event, shape_idx):
	if Input.is_action_just_pressed("click"):
		if made_instance == false:
			made_instance = true
			root_scene.add_child(shopping_section_instance)
			shopping_section_icon_node.hide()
			shopping_section_instance.position = get_global_mouse_position()
	if Input.is_action_just_released("click"):
		if made_instance == true and area_entered == true:
			made_instance = false
			root_scene.remove_child(shopping_section_instance)
			shopping_section_icon_node.show()
			shopping_section_instance.delete_instance()
