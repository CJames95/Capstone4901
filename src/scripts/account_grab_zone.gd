...
var account = preload("res://src/scenes/account_piece.tscn")
var root_scene
var account_icon_node
...
func _ready():
	...
	account_instance = account.instantiate()
	account_icon_node = get_node("Small Account")
...
func _on_area_2d_input_event(viewport, event, shape_idx):
	if Input.is_action_just_pressed("click"):
		if made_instance == false:
			made_instance = true
			root_scene.add_child(account_instance)
			account_icon_node.hide()
			account_instance.position = get_global_mouse_position()
	if Input.is_action_just_released("click"):
		if made_instance == true and area_entered == true:
			made_instance = false
			root_scene.remove_child(account_instance)
			account_icon_node.show()
			account_instance.delete_instance()
